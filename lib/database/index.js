...
Database.prototype.configure = function configure(collectionName, schema, meta) {
  var name = collectionName.toLowerCase();
  this.schema.registerCollection(name, schema, meta);
};
...
Database.prototype.create = function create(collectionName, data, cb) {
  ...
  var self = this,
      ...
      ttl = self.schema._ttl[name];

  //removes spaces if the string just in case there is some, as redis doesn't like it...
  ...
  async.auto({
    ...
    saveKeyData: ['setSequenceValue', function(next, results) {
      ...
      if ( ttl > 0){
        //if it has a ttl over 0 then do the setex ( set expiry ) redis-comand
        self.connection.setex(noSqlKey, ttl, parsedData, function(err){
          if(err) return next(err);
